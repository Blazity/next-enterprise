<script lang="ts">
	...
	import { eases, types } from '$lib/utils';
	...
	const current = $derived(() => {
		const easeGroup = eases.get(current_ease);
		const type = current_type as 'In' | 'Out' | 'InOut';
		return easeGroup?.[type];
	});
	...
	async function runAnimations() {
		...
		if (currentValue) {
			await ease_path.set(currentValue.shape);
			await Promise.all([
				time.set(1000, { duration, easing: (x) => x }),
				value.set(0, { duration, easing: currentValue.fn })
			]);
		}
