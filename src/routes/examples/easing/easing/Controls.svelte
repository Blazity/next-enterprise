...
<div class="easing-sidebar">
			...
			<ul>
				{#each [...eases] as [name]}
					<li class:selected={name === current_ease}>
						<button type="button" onclick={() => (current_ease = name)}> {name}</button>
					</li>
				{/each}
			</ul>
		{/if}
			...
			<ul>
				{#each types as [name, type]}
					<li class:selected={type === current_type}>
						<button type="button" onclick={() => (current_type = type)}> {name}</button>
					</li>
				{/each}
			</ul>
		{/if}
	...
	<div class="duration">
		<span>
			...
			<button type="button" class="number" onclick={() => (duration -= 100)}>-</button>
			<button type="button" class="number" onclick={() => (duration += 100)}>+</button>
		</span>
		<button type="button" class="play" onclick={() => dispatch('play')}>
